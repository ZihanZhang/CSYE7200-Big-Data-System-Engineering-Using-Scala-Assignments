...
trait RandomState[T] {
  ...
  /**
    ...
    * @return a new random state
    */
  def flatMap[U](f: T=>RandomState[U]): RandomState[U] = f(get: T)// TODO 10 points
...
case class JavaRandomState[T](n: Long, g: Long=>T) extends RandomState[T] {
  def next: RandomState[T] = JavaRandomState(new Random(n).nextLong(): Long ,g: Long=>T) // TODO 7 points
  def get: T = g(n: Long) // TODO 5 points
  def map[U](f: (T) => U): RandomState[U] = JavaRandomState(n, g.andThen(f)) // TODO 13 points
}
...
object RandomState {
  ...
  val longToDouble: Long=>Double = {x => x.toDouble/Long.MaxValue} // TODO 4 points
