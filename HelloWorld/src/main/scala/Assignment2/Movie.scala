...
object Movie extends App {
  ...
  trait IngestibleMovie extends Ingestible[Movie] {
    def fromString(w: String): Try[Movie] = Try(apply(Seq(w))) // TODO 11 points
  }
  ...
  def elements(list: Seq[String], indices: Int*): List[String] = {
    ...
    for (i <- 1 to indices.length) x ++ list(indices(i))
    // TODO 6 points
    x.toList
  }
...
object Rating {
  ...
  /**
    ...
    * @return a Rating
    */
  def apply(s: String): Rating = new Rating(rRating.findFirstIn(s).mkString,None)  // TODO 13 points
}