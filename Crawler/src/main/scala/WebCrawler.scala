...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    def getURLs(ns: Node): Seq[URL] = for (cn <- ns \\ "a") yield new URL(u, cn.attributes.mkString)
    ...
    for (a <- getURLContent(u); c <- MonadOps.asFuture(getLinks(a))) yield c
  }
